...
namespace AlbumOnline
{
    ...
    public class AlbumSW : System.Web.Services.WebService
    {
        ...
        {
            return db.Photos.Where(x => x.TipoId == tipo && x.Status == "A").Select(x => new
           SWPhoto()
            {
                Id = x.Id,
                Img = x.Img,
        ...
        // var result = SignInManager.PasswordSignInAsync(user, pass, false, shouldLockout:false);
            var result =
           HttpContext.Current.GetOwinContext().Get<ApplicationSignInManager>().PasswordSignIn(user, pass,
           false, false);
